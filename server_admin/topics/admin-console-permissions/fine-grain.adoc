...
In this example, we are creating a `User Policy` for the `manage` permission. Upon initialization, the `manage` permission has no policies associated with it. We must create these policies.
...
The `query-clients` role must be assigned to `sales-admin` so the Admin Console renders client menus for the `sales-admin` user.

[IMPORTANT]
====
If the `query-clients` role is unset, restricted admins do not see any menu options when they log into the Admin Console.
====
...
A short way to do this is to grant access to the `map-roles` to an admin. Admins can then map any role defined by the client.
+
...
This is currently a limitation of protocol mappers. They cannot assign individual permissions as roles do.
====
...
===== map-roles
A subset of the privileges granted by the `manage` scope. In this case the ability to map roles. The admin cannot perform any other user management operation. This is similar to the `manage` role. When dealing with client roles, the roles the admin applies must be specified per role, or set of roles.
