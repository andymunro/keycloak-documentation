...
===== map-roles
A subset of the privileges granted by the `manage` scope. In this case the ability to map roles. The admin cannot perform any other user management operation. This role is similar to the `manage` role. When dealing with client roles, the roles the admin applies must be specified per role, or set of roles.
