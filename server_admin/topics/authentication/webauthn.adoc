...
[NOTE]
====
WebAuthn's operations success depends on the users WebAuthn supporting authenticator, browser, and platform. Make sure your authenticator, browser, and platform support the WebAuthn specification.
====
...
Mark the `Default Action` checkbox if you want all newly created users to be required to register their WebAuthn credentials.
...
. Click *Ok*.
. Click on the _Actions_ link for `WebAuthn Browser Browser - Conditional OTP` and click _Delete_.
. Click *Delete*.
...
[NOTE]
====
If a user does not have WebAuthn credentials, the user will be forced to register WebAuthn credentials.
====
...
|The readable server name as a WebAuthn Relying Party. This item is mandatory. It applies to the registration of the WebAuthn authenticator. The default setting is "keycloak". For more details, see https://www.w3.org/TR/webauthn/#dictionary-pkcredentialentity[WebAuthn Specification].
...
|The Option requiring that the WebAuthn authenticator generates the Public Key Credential as https://www.w3.org/TR/webauthn/#client-side-resident-public-key-credential-source[Client-side-resident Public Key Credential Source]. This is an optional configuration item applying to the registration of the WebAuthn authenticator. If no option exists, its behavior is the same as selecting "No". For more details, see https://www.w3.org/TR/webauthn/#dom-authenticatorselectioncriteria-requireresidentkey[WebAuthn Specification].
...
|If enabled, the already registered WebAuthn authenticators cannot be reregistered.

|Acceptable AAGUIDs
|The white list of AAGUIDs which a WebAuthn authenticator registers against.
...
If the `WebAuthn Register` required action is set as `Default Action` in a realm, new users must set up the WebAuthn security key after their first login. 

.Procedure

. Open the login form.
. Click `Register`.
. Fill in the items on the register form.
. Click `Register`.

After successfully registering, the browser asks the user to enter the text of their, just registered, WebAuthn authenticator's label.

===== Existing user

If `WebAuthn Authenticator` is set up, existing users must register their WebAuthn authenticator when they attempt to log in.

.Procedure
. Open the login form.
. Fill in items on the form.
. Click *Save*.
. Click *Login*.

After successful registration, the users browser asks the user to enter the text of their, just registered, WebAuthn authenticators label.

==== Passwordless WebAuthn together with Two-Factor

WebAuthn is used for two-factor authentication but it is possible to use WebAuthn as first factor authentication also. In this case, users with `passwordless` WebAuthn credentials can  authenticate to {project_name} without a password. {project_name} can use WebAuthn as both the passwordless and two-factor authentication mechanism in the context of a realm and in the context of a single authentication flow.

An administrator typically requires that Security Keys registered by users for the WebAuthn passwordless authentication must meet different requirements. For example, the security keys may require users to authenticate to the security key using a PIN, or the security key is attested with stronger certificate authority.
...
. Register new required action for WebAuthn passwordless support. Use the steps described in <<_webauthn-register, Enable Webauthn Authenticator Registration>>. You must register the `Webauthn Register Passwordless` action.

. Configure the policy. You can use steps and configuration options described in <<_webauthn-policy, Managing Policy>>. You must perform the configuration in the admin console in the tab `WebAuthn Passwordless Policy`. Typically the requirements for the security key will be stronger than for the two-factor policy. For example you can set the `User Verification Requirement` to `Required` when you configure the passwordless policy.
...
+
** The `WebAuthn Browser Forms` subflow contains `Username Form` as the first authenticator. Delete the default `Username Password Form` authenticator and add the `Username Form` authenticator. This requires the user to provide their username as the first step.
+
** There will be a required subflow, which can be named `Passwordless Or Two-factor`, for example. This indicates the user can authenticate either with Passwordless WebAuthn credential or with Two-factor authentication.
+
** Flow contains `WebAuthn Passwordless Authenticator` as the first alternative.
+
...
You can now add `WebAuthn Register Passwordless` as the required action to a user, already known to {project_name}, to test this. During the first authentication, the user must use the password and second-factor WebAuthn credential. If the user uses their WebAuthn Passwordless credential, they do not need to provide the password and second-factor WebAuthn credential.
