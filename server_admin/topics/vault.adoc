...
{project_name} combines the realm name with the `_key_` obtained from the vault expression to prevent secrets from leaking across realms. This method means that the `_key_` does not directly map to an entry in the vault but creates the final entry name according to the algorithm used to combine the `_key_` with the realm name.
...
To use a vault, you must register a vault provider in {project_name}. You can use the providers described <<_providers, here>> or implement your provider. See the link:{developerguide_link}[{developerguide_name}] for more information.

[NOTE]
====
{project_name} permits a maximum of one active vault provider per {project_name} instance at a time. You must configure the vault provider in each instance within the cluster consistently.
====
...
A keystore backs this credential store. `JCEKS` is the default format, but it is possible to use other formats such as `PKCS12`. Users can create and manage the store contents using the `elytron` subsystem in WildFly/JBoss EAP, or the `elytron-tool.sh` script.
...
If the underlying keystore has a format different than `JCEKS`, you must specify this format by using the `keyStoreType`:
...
The custom factory returns a key resolver that combines the realm and key with a triple `#` character. For example, an entry would look like `master_realm###smtp_key`. You must then install this factory like any custom provider.
