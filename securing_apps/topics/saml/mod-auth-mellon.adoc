...
To configure mod_auth_mellon you will need:
...
To configure the Mellon service provider, create the file /etc/httpd/conf.d/mellon.conf with this content:
...
* The {project_name} IdP is installed on the $idp_host.

* You have access to the Admin Console and the admin username and password for $idp_host.

.Procedure

. Log into the Admin Console using the admin username and password.
+
...
. Click *Clients* in the  menu item

. Click *Create* in the upper right corner to create a new client.
